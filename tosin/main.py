...
DATABASE_URL = "postgresql://postgres:!post123!@72.52.132.11:5432/\
    test_db_educify"
...
def recommend_teachers_with_ai(db: Session, student: Dict[str, Any]):
    ...
    teachers = db.query(Teacher.id, Lesson.title, Teacher.rating,
                        Lesson.travelDistance)\
    ...
    try:
        ...
        for rec in recommended:
            notification = TeacherNotification(
                booking_id=booking.id, teacher_id=rec["teacher_id"],
                active=True)
            db.add(notification)
            db.commit()
...
async def teacher_response(response: TeacherResponse,
    ...
    try:
        ...
        if response.response.lower() == "accept":
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid response. Use 'accept' or 'deny'.")
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
